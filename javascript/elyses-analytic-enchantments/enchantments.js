...
export function getCardPosition(stack, card) {
    return stack.findIndex((theCard) => theCard == card);
}
...
export function getFirstOddCard(stack) {
    let oddCard = stack.findIndex((card) => card % 2 !== 0);
    return oddCard != -1 ? stack[oddCard] : undefined;
}
...
export function getFirstEvenCardPosition(stack) {
    let evenCard = stack.findIndex((card) => card %2 == 0);
    return evenCard;
}
